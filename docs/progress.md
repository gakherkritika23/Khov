...
| `tests/contactUsPage.spec.ts` | TC-01..TC-05 — per interest: footer → Contact Us, verify fields exist, log dropdown options, required + invalid email/phone validation, fill + submit (success + /api/contact-us/ 200; prod fill-only) | @form @smoke/@regression | Contact Us page (5 forms) |
...
1. **E6** — multi-surface contact (QMI / floorplan detail "Request Information"), if desired beyond `qmiPage`'s coverage.
